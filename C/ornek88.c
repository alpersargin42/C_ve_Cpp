...
void dosya_olustur(){
    ...
    if(file.dosya_ismi == NULL || file.dosya_yolu == NULL){
        printf("Dosya adı ve dosya yolu bilgilerini eksiksiz doldurun");
        return;
    }
...
void arama(){
    ...
    printf("Verilen isimde klasör yada belge bulunamadı: %s\n", isim);
}
...
void sil(){
    ...
    printf("Silinecek belgenin adını giriniz:");scanf("%s",dosyaIsmi);
    strcat(filePath, yol);
    strcat(filePath, dosyaIsmi);
    int del = remove(filePath);
    if (!del)
        printf("Dosya silmesi başarılı");
    else
